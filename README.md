...
Plugin collection, Desktop Plugin source, and Marketplace catalog for DSH-DeskTop.

Repository: <https://github.com/run-bigpig/dsh-plugin-hub>

The repository has two independently released surfaces:

- `packages/plugin-*`: trusted Host, Client, and profile Bundle code shipped with the desktop application.
- `catalog/`: signed metadata pointing at immutable, prebuilt GitHub Release assets.

Desktop Plugin code is built against the exact Harness commit embedded by the desktop application. Catalog updates never replace the trusted Desktop Plugin Bundle.

## Layout

```text
packages/plugin-host/         Host Remote gateway to the desktop control bridge
packages/plugin-client/       Harness desktop UI and Marketplace contributions
packages/plugin-bundle/       Profile patch mounting both packages
...
DSH-DeskTop stages these artifacts on Windows with:

```powershell
.\scripts\prepare-windows-seed.ps1 -PluginSource E:\path\to\deepseek-harness-plugins
```

`-PluginSource` may also be supplied through `DSH_DESKTOP_PLUGIN_SOURCE`. The former parameter and environment variable remain accepted for build compatibility. The desktop build never copies this repository or its `node_modules`; only the packed Desktop Plugin artifacts, generated catalog, and optional detached signature are staged.
